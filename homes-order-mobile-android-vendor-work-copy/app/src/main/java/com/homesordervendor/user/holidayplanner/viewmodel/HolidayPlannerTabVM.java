...
public class HolidayPlannerTabVM extends java.util.Observable{

    Activity activity;
    MyProgressDialog myProgressDialog;
    List<HolidayPlanner> holidayPlannerList=new ArrayList<>();

    public HolidayPlannerTabVM(Activity activity){
        this.activity=activity;
        myProgressDialog=new MyProgressDialog();
        getHolidayPlannerAPICall();
    }

    public List<HolidayPlanner> getHolidayPlannerList(){
        return  holidayPlannerList;
    }

    public void getHolidayPlannerAPICall(){
        if (InternetChecker.getInstance().isReachable()) {
            myProgressDialog.showDialog(activity);
            APICall api = APIConfiguration.getInstance().createService(APICall.class);
            Observable<Response<HolidayPlannerResponse>> observable = api.getOffDayPlan(MySession
                    .getInstance
            ...
            observable.subscribeOn(Schedulers.io())
                    ...
                    .subscribe(responses -> {
                        ...
                        if (responses.code() == 200) {
                            holidayPlannerList=responses.body().getOffDayPlans();
                        } else {
