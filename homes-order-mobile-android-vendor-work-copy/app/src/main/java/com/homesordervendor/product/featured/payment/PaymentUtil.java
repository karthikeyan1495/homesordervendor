package com.homesordervendor.product.featured.payment;

import android.app.Activity;
import android.content.Intent;
import android.provider.Settings;

import com.homesordervendor.BuildConfig;
...
import static com.homesordervendor.MyApp.getContext;
...
public class PaymentUtil {
    ...
    public static void telrPaymentGatewayInitialization(Activity activity, String amount){
        MobileRequest mobileRequest=telrPaymentRequest(activity, amount);
        if (mobileRequest!=null) {
            Intent intent = new Intent(activity, WebviewActivity.class);
            intent.addFlags(Intent.FLAG_ACTIVITY_BROUGHT_TO_FRONT);
            // intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            intent.putExtra(WebviewActivity.EXTRA_MESSAGE, mobileRequest);
            intent.putExtra(WebviewActivity.SUCCESS_ACTIVTY_CLASS_NAME, "com.homesordervendor.product.featured.payment.TelrPaymentSuccessActivity");
            intent.putExtra(WebviewActivity.FAILED_ACTIVTY_CLASS_NAME, "com.homesordervendor.product.featured.payment.TelrPaymentFailedActivity");
    ...
    private static MobileRequest telrPaymentRequest(Activity activity, String amount) {
        try {
            ...
            address.setCity(MySession.getInstance(activity).getUser().getProfile().getAddress().getCity());
            //
            // City :
            // the
            // minimum
            ...
            address.setCountry(MySession.getInstance(activity).getUser().getProfile().getAddress
                    ().getCountryId());                       //
            // Country : Country must
            // be sent as a 2
            ...
            address.setLine1(MySession.getInstance(activity).getUser().getProfile().getAddress()
                    .getStreet());                 // Street
            // address – line 1: the
            // minimum required
           ...
           // address.setLine2("deliveryAddress.getAddress_line2()");               // (Optinal)
            ...
            name.setFirst(MySession.getInstance(activity).getUser().getProfile().getFullname());
            // Forename
            // : the minimum
            // required details
            ...
            billing.setEmail(MySession.getInstance(activity).getUser().getProfile().getEmail());
             // TODO: Insert
            // your email
            // here : the
