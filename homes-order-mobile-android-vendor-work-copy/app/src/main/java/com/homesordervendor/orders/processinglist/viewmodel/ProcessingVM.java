package com.homesordervendor.orders.processinglist.viewmodel;
...
/**
 * Created by innoppl on 09/03/18.
 */

public class ProcessingVM extends java.util.Observable{
    ...
    public ProcessingVM(Activity activity){
    ...
    public void orderListAPICall(){
        if (InternetChecker.getInstance().isReachable()) {
            ...
            Observable<Response<List<OrderItem>>> observable = api.orderList(MySession
                    .getInstance(activity).getUser().getToken(), OrderStatus.PROCESSING.getValue());
